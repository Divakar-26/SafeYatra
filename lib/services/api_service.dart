  ...
  class ApiService {
    static const String baseUrl = "https://08a51c58c267.ngrok-free.app";
    ...
    static Future<Map<String, dynamic>> updatePrivacySettings(
      ...
      final uri = Uri.parse('$baseUrl/privacy/$email');
      try {
        final resp = await http
            .put(
          uri,
        ...
        if (resp.statusCode == 200) {
          return {'success': true, 'message': data['message'] ?? 'Privacy settings updated'};
        }
        ...
        return {
          ...
          'message': data['detail']?.toString() ??
              data['raw']?.toString() ??
              'Failed to update privacy settings (${resp.statusCode})',
        };
      } on Exception catch (e) {
        return {'success': false, 'message': 'Network error: $e'};
      }
    ...
    static Future<Map<String, dynamic>> getPrivacySettings(String email) async {
      final uri = Uri.parse('$baseUrl/privacy/$email');
      try {
        ...
        if (resp.statusCode == 200) {
          return {
            'success': true,
            'share_health': data['share_health'] ?? false,
        ...
        return {
          ...
          'message': data['detail']?.toString() ??
              data['raw']?.toString() ??
              'Failed to get privacy settings (${resp.statusCode})',
        };
      } on Exception catch (e) {
        return {'success': false, 'message': 'Network error: $e'};
      }
